...
import { IServiceInstance, ServiceSchema, ServiceState, ILogger } from '../types/registry.types';
...
export class ServiceLifecycle extends EventEmitter {
    ...
    constructor(
        ...
        super();
        this.registry.on('changed', () => this.evaluateDependencies());
    }
    ...
    private async evaluateDependencies(): Promise<void> {
        ...
        for (const service of this.services.values()) {
            ...
            const missingDeps = deps.filter(dep => !availableServices.has(dep));
